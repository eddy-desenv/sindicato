#include "rwmake.ch"    
User Function boletogr(WPAR1,WPAR2)        // incluido pelo assistente de conversao do AP5 IDE em 16/10/00
...
Do While SE1->E1_FILIAL == xFilial("SE1") .And. SE1->E1_PREFIXO <= MV_PAR02 .And.;
   ...
   If SE1->E1_EMISSAO < MV_PAR05 .Or. SE1->E1_EMISSAO > MV_PAR06 .Or.;
      ...
      SE1->E1_LOJA    < MV_PAR10 .Or. SE1->E1_LOJA    > MV_PAR12 //   Val(SE1->E1_IMPRESS) # MV_PAR13 .Or. SE1->E1_TIPODOC <> "BO" .Or. SE1->E1_PORTADOR == "777"
      DbSkip()
      Loop
   EndIf
...
*/
   ...
   Else
      DbSeek(xFilial("SA6")+TRIM(mv_par18),.F.)
   EndIf
   DbSelectArea("SEE")       
   Dbsetorder(1)
...
//   EndIf
   ...
   If !Empty(SA1->A1_ENDCOB)
      ...
      TAM  := 0
      TAM  := 81 - LEN(wTAM)
      @ 04,068 PSAY "≥  Endereáo : " + Alltrim(SA1->A1_ENDCOB)+ SPACE(TAM) // + " " + Alltrim(SA1->A1_NUMENDC) 
   Else
      wTAM := SPACE(81)
      wTAM := "≥  Endereáo : " + Alltrim(SA1->A1_END)// + " " + Alltrim(SA1->A1_NUMEND)
      TAM  := 0
      TAM  := 81 - LEN(wTAM)
      @ 04,068 PSAY "≥  Endereáo : " + Alltrim(SA1->A1_END)+ SPACE(TAM) //+ " " + Alltrim(SA1->A1_NUMEND)  + SPACE(TAM)
   EndIf
   @ 04,148 PSAY " ≥"
...
//      @ 57,015 PSAY Trim(SA1->A1_ENDCOB) + " " + SA1->A1_NUMENDC + " - " + xCOMPLEM+AllTrim(SA1->A1_BAIRROC)+" - "+AllTrim(SA1->A1_MUNC)+" - "+AllTrim(SA1->A1_ESTC)+" - "+ Transform(SA1->A1_CEPC,"@R 99999-999")
      @ 57,015 PSAY Trim(SA1->A1_ENDCOB) + " - " + xCOMPLEM+AllTrim(SA1->A1_BAIRROC)+" - "+AllTrim(SA1->A1_MUNC)+" - "+AllTrim(SA1->A1_ESTC)+" - "+ Transform(SA1->A1_CEPC,"@R 99999-999")
   Else
//      @ 57,015 PSAY Trim(SA1->A1_END) + " " + SA1->A1_NUMEND + xCOMPLEM + " - " + AllTrim(SA1->A1_BAIRRO)+" - "+AllTrim(SA1->A1_MUN)+" - "+AllTrim(SA1->A1_EST)+" - "+ Transform(SA1->A1_CEP,"@R 99999-999")
      @ 57,015 PSAY Trim(SA1->A1_END) + " "+ xCOMPLEM + " - " + AllTrim(SA1->A1_BAIRRO)+" - "+AllTrim(SA1->A1_MUN)+" - "+AllTrim(SA1->A1_EST)+" - "+ Transform(SA1->A1_CEP,"@R 99999-999")
   EndIf
...
//   @ PROW()+1,0 MSBAR(cTypeBar,nRow,nCol,cCode,oPrint,lCheck,Color,lHorz,nWidth,nHeigth,lBanner,cFont,cMode)
   DbSelectArea("SE1")
/*   If SE1->E1_IMPRESS == " "
      RecLock("SE1",.F.)
      SE1->E1_IMPRESS := "1"
...
aAdd(aRegs,{cPerg,"18","Banco                  ","","","mv_chi","C",03,0,0,"G","","mv_par18","","","","","","","","","","","","","","","","","","","","","","","","","","","","","",""})
