...
ฑฑบ          ณMedica da base historica existente antes da melhoria do     บฑฑ
ฑฑบ          ณPlano Assistencia Medica / Odontologica.                    บฑฑ
ฑฑบ          ณ Apos gerar o reteio sera possivel tambem efetuar manutencaoบฑฑ
ฑฑบ          ณdos valores gerados no rateio.                              บฑฑ
...
aAdd(aRegs,{cPerg,"01","Filial De          ?","จDe Sucursal       ?","From Branch        ?","mv_ch1","C",02,0,0,"G","Naovazio"		,"mv_par01",""				,"","","01","","","","","","","","","","","","","","","","","","","","","SM0","" })
aAdd(aRegs,{cPerg,"02","Filial At         ?","จA  Sucursal       ?","To Branch          ?","mv_ch2","C",02,0,0,"G","Naovazio"		,"mv_par02",""				,"","","99","","","","","","","","","","","","","","","","","","","","","SM0","" })
...
if cTpAssMed == "1" //TIPO 1 (Tabela 22)
	...
	elseif SRX->( dbSeek(Space(2)+"22") )
		cChaveSRX := Space(2)+"22"
	endif
...
elseif cTpAssMed == "2" //TIPO 2 (Tabela 58)
	...
	elseif SRX->( dbSeek(Space(2)+"58") )
		cChaveSRX := Space(2)+"58"
	endif
...
Local lRet 		:= .T.
Local cFilSRB 	:= if(empty(xFilial("SRB")),Space(2),(cAliasSRA)->RA_FILIAL)
Local nIdade  	:= 0
Local lIsDepAM  := .F.
...
for nX:= 1 to len(aCols)
	...
	for nY:=1 to len(aHeader)
		...
		if (cAliasArq)->( dbSeek(SRA->RA_FILIAL+SRA->RA_MAT+cCodDep+cAno+cMes) )
			...
			if aCols[nX][nY] > 0 //Soh insere novo registro se tiver valor informado
				RecLock(cAliasArq, .T.)
				(cAliasArq)->FILIAL 	:= SRA->RA_FILIAL
				(cAliasArq)->MATFUNC	:= SRA->RA_MAT
				(cAliasArq)->ANO 		:= cAno
				(cAliasArq)->MES 		:= cMes
				(cAliasArq)->CODDEP 	:= cCodDep
				(cAliasArq)->CPF		:= cCPF
				(cAliasArq)->VALOR 		:= aCols[nX][nY]
				(cAliasArq)->( MsUnLock() )
			endif
