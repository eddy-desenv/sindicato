...
IF !EMPTY(::CROTINA) .AND. ALLTRIM(::CROTINA)=='3' //INCLUSAO                      
	...
	cDDD := SUBSTR(::AEMPRESA:cTel, 1, 4)
   	cDDD := STRTRAN(cDDD,"(","")
   	cDDD := AllTrim(STRTRAN(cDDD,")",""))
	cTel := AllTrim(SUBSTR(::AEMPRESA:cTel, 5, 15))	      	    	      	    
	...
	aadd(aDados,{"A1_DDD"    		,cDDD									,nil})
	aadd(aDados,{"A1_TEL"    		,cTel									,nil})
...
ELSEIF !EMPTY(::CROTINA) .AND. ALLTRIM(::CROTINA)=='4' //ALTERACAO
	...
	CUF			:= SA1->A1_EST
	CEMAIL		:= SA1->A1_EMAIL
	CTEL		:= IIF (!Empty(SA1->A1_DDD),SA1->A1_DDD + SA1->A1_TEL,SA1->A1_TEL)
	CFAX		:= SA1->A1_FAX
...
IF !EMPTY(::CCGC)
	...
	ELSE
		...
		IF SA1->(dbSeek(xFilial("SA1") + CCGC_))  //LOCALIZOU 
			::AEMPRESA := WSCLASSNEW('StrEmpresa')
			...
			::AEMPRESA:CTEL			  := IIF(!Empty(SA1->A1_DDD),SA1->A1_DDD+SA1->A1_TEL,SA1->A1_TEL)
...
IF CEP->(DbSeek(xFilial("CEP")+::CEP)) 
    LVALID := .T.
	::AEND:ENDERECO	:= Alltrim(Upper(FWNoAccent(CEP->CEP_END)))
	::AEND:BAIRRO   	:= Upper(FWNoAccent(CEP->CEP_BAIRRO))
	::AEND:CEP      	:= CEP->CEP_NUM
	::AEND:EST      	:= Upper(CEP->CEP_EST)
	::AEND:ERSIN    	:= Posicione("SZO",1,xFilial("SZO")+Upper(FWNoAccent(Alltrim(CEP->CEP_MUN))),"ZO_ERSIN")
	::AEND:MUN      	:= Upper(FWNoAccent(SZO->ZO_MUN))
