...
³ MV_PAR18   // 2a. Parcela a considerar baixa			 	     ³                  G
³ mv_par19   // Data de Cadastro ate ?                                       ³
³ MV_PAR20   // Codigo Do Sindicato                                            ³
...
//			!(dbseek( xFilial()+strzero(mv_par01,2)+"A"+SA1->A1_COD+mv_par02+"DP"))
	...
	DETA->COD_BANCO := "237"
	DETA->ZEROS     := "00000"
	_NossoNum := strzero(val(IIF( SX5->( DbSeek( xFilial( "SX5" ) + "GR" + "06" ) ), STRZERO(VAL(SX5->X5_DESCSPA),4), "0000" )),4) + strzero(val(_nseq),7)
...
IF SX5->( DbSeek( xFilial( "SX5" ) + "GR" + "06" ) )
   RecLock( "SX5", .F. )
...
ARQ := "\arquivos\assistencial\"+MV_PAR04
COPY TO &ARQ SDF
dbSelectArea("MALA")
ARQ := "\arquivos\assistencial\"+MV_PAR16
