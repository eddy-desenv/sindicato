...
if TMP->COD_REGIST == "0"  //registro Header
	...
	while !eof() .and. TMP->COD_REGIST == "1"
		...
		if _cConta $ "0169641/0180013/0180015/0180061/0180057/0180011"    && C/C Assistencial
			_cLote:="7555"	 && Lote para ASSISTENCIAL
			XNATUREZA := "0301"
		elseif _cConta $ "0162159/0180010/0180012/0180014/0180055/0180059"    && C/C Termos
		...
		if  substr(TMP->CAMPO,108,2) =="06" //LIQUIDACAO NORMAL
			...
			if eof() .and. _cCGC <> "99"	  && Gera titulo se nao existe (Recepcao normal)
				...
				if _altconf
					...
					SE1->E1_STATUS  := "B"
					if .T. // Nilton - 10/11/08 - // se1->e1_valor > 0 .and. se1->e1_valor < 0.02
						SE1->E1_valor   := val( substr(TMP->CAMPO,253,13) )/100
						SE1->E1_SALDO   := 0
					endif
